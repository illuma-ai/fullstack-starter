...
window.DashboardPage = function DashboardPage() {
  ...
  if (auth.loading) {
    return h(
      ...
      h("div", {
        className:
          "w-6 h-6 border-2 border-primary-500 border-t-transparent rounded-full animate-spin",
      }),
    );
  }
  ...
  return h(
    "div",
    { className: "max-w-5xl mx-auto px-4 sm:px-6 py-6 fade-in" },
    // Header
    h(
      "div",
      ...
      h(
        "button",
        {
          ...
          className:
            "px-4 py-2 bg-primary-600 text-white text-sm rounded-lg hover:bg-primary-700 font-medium",
        },
        "+ New Item",
      ),
    ),
    ...
    h("div", { className: "mb-6" }, h(window.StatsChart, { stats: stats })),
    ...
    app.loading
      ? h(
          ...
          h("div", {
            className:
              "w-6 h-6 border-2 border-primary-500 border-t-transparent rounded-full animate-spin mx-auto mb-2",
          }),
      ...
      : app.items.length === 0
        ...
        : h(
            "div",
            { className: "grid gap-3 " + (showForm ? "mt-4" : "") },
...
function _filterSelect(label, value, options, onChange) {
  return h(
    "select",
    {
      ...
      className:
        "px-3 py-1.5 border border-gray-300 rounded-lg text-sm text-gray-600 focus:outline-none focus:ring-2 focus:ring-primary-500",
    },
